...
export async function enrichThreadsForUser<T extends { id: string }>(
  prisma: PrismaService,
  threads: T[],
  userId: string,
): Promise<(T & { userReaction: string | null; isSaved: boolean })[]> {
  ...
  const [reactions, saved] = await Promise.all([
    prisma.threadReaction.findMany({
      where: { userId, threadId: { in: threadIds } },
      select: { threadId: true, reaction: true },
      take: 50,
    }),
    prisma.threadBookmark.findMany({
      where: { userId, threadId: { in: threadIds } },
      select: { threadId: true },
      take: 50,
    }),
  ]);
  const reactionMap = new Map(reactions.map(r => [r.threadId, r.reaction]));
  const savedSet = new Set(saved.map(s => s.threadId));
...
export async function enrichVideosForUser<T extends { id: string }>(
  prisma: PrismaService,
  videos: T[],
  userId: string,
): Promise<(T & { userReaction: string | null; isSaved: boolean })[]> {
  ...
  return videos.map(video => ({
    ...video,
    userReaction: reactionMap.get(video.id) ?? null,
