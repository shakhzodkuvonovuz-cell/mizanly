...
- [Database Schema](#database-schema-210-models-112-enums)
- [WebSocket Gateway](#websocket-gateway)
- [Internationalization](#internationalization-i18n)
- [All Screens](#all-screens-218-total)
...
| Prisma Models | 210 |
| Prisma Enums | 112 |
| Tests Passing | **7,256** (API + Signal 671 + Go E2E 57 + Go LiveKit 131) |
| UI Components | 74 |
| Custom Hooks | 42 (incl. conversation 4, video-editor 6, create-screen 8) |
| Mobile Services | 52 |
| Signal Protocol Files | 23 files, ~10,310 lines |
| Go E2E Key Server | 13 endpoints, 57 tests |
| Go LiveKit Call Server | 16 endpoints, 131 tests |
| Translation Keys | 4,900+ per language |
| Supported Languages | 8 (en, ar, tr, ur, bn, fr, id, ms) |
| Story Sticker Types | 10 interactive + 2 static |
| Audit Findings Fixed | ~7,000+ across 20 sessions |
| E2E Security Audit | All waves closed, A+ grade |
| MASTER_DEFERRED | **1,456 items → 0 code-fixable remaining** |
| RTL Support | Complete (all screens, 0 marginLeft/Right outside image-editor) |
| Accessibility | 724 interactive elements labeled, all 8 languages |
| Architecture | Event-driven notifications (21/21), 0 circular deps, 0 god components |
| Git Commits | 1,662+ |
| Development Time | 33 days (Mar 3 – Apr 4, 2026) |
...
| **State** | Zustand | 5.0 | 8 domain slices (auth, ui, feed, media, chat, settings, network, call) |
...
│       │   ├── store/               # Zustand store (8 domain slices)
│       │   ├── theme/               # Design tokens
...
42 hooks in `apps/mobile/src/hooks/`:
...
| `useDraftPersistence` | `useDraftPersistence.ts` | MMKV-backed draft save/load/clear with debounced auto-save |
| `useIsOffline` | `useIsOffline.ts` | NetInfo-based offline detection for UI guards |
| **Conversation Hooks** | `conversation/` | Extracted from 3,290-line god component |
| `useConversationMessages` | `conversation/useConversationMessages.ts` | Message queries, pagination, socket listeners, offline queue |
| `useMessageSend` | `conversation/useMessageSend.ts` | E2E encrypt, media send, typing, sealed sender |
| `useVoiceRecording` | `conversation/useVoiceRecording.ts` | Audio recording, permissions, encrypted upload |
| `useConversationEncryption` | `conversation/useConversationEncryption.ts` | Decrypt 3 message types, cache, disappearing timer |
| **Video Editor Hooks** | `video-editor/` | Extracted from 2,606-line god component |
| `useVideoEditorState` | `video-editor/useVideoEditorState.ts` | All editable state, undo/redo stack |
| `useVideoPlayback` | `video-editor/useVideoPlayback.ts` | Playback position, status updates |
| `useVideoTimeline` | `video-editor/useVideoTimeline.ts` | Trim markers, scrubbing |
| `useVideoExport` | `video-editor/useVideoExport.ts` | FFmpeg commands, progress, output |
| `useVideoVoiceover` | `video-editor/useVideoVoiceover.ts` | Recording, permissions, voice effects |
| `useVideoVolumeGestures` | `video-editor/useVideoVolumeGestures.ts` | Pan gesture volume control |
| **Create Screen Hooks** | `create/` | Extracted from 3 god create screens (4,677 → 2,665 lines) |
| `useStoryCapture` | `create/useStoryCapture.ts` | Camera/gallery selection, media state |
| `useStoryEffects` | `create/useStoryEffects.ts` | Filters, text overlay, stickers, drawing, music |
| `useStoryPublish` | `create/useStoryPublish.ts` | R2 upload, API call, close-friends toggle |
| `useReelCapture` | `create/useReelCapture.ts` | Camera recording, multi-clip, FFmpeg concat |
| `useReelEdit` | `create/useReelEdit.ts` | Caption, hashtags, countdown, draft, music |
| `useReelPublish` | `create/useReelPublish.ts` | R2 upload, API call, discard confirmation |
| `usePostMedia` | `create/usePostMedia.ts` | Image/video picking, NSFW check, carousel |
| `usePostPublish` | `create/usePostPublish.ts` | Visibility, upload, all publish fields, autocomplete |
...
## Database Schema (210 Models, 112 Enums)
